...
def test_readiness_healthy_environment():
    ...
    with patch("app.startup_validation.get_config", return_value="healthy-key"):
...
def test_readiness_db_config_lookup_failure_returns_503(monkeypatch):
    monkeypatch.delenv("NL2SQL_API_KEY", raising=False)
    monkeypatch.setattr("app.startup_validation.get_config", lambda key: (_ for _ in ()).throw(Exception("db down")))
    response = client.get("/ready")
    assert response.status_code == 503
