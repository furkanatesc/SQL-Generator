...
def test_retriever_rejects_invalid_allowed_object_type():
    retriever = InMemoryTopKRetriever()
    query = RetrievalQuery(
        query_text="test",
        query_vector=(1.0, 0.0),
        ...
        model_id="fake_model",
        dimension=2,
        allowed_object_types=("table", "banana")  # banana is invalid
    )
    record = build_mock_record("table:test", (1.0, 0.0), dimension=2)
    with pytest.raises(EmbeddingNonRetryableError, match="Invalid allowed_object_types: \\['banana'\\]"):
        retriever.retrieve(query, [record])

def test_retriever_rejects_query_vector_dimension_mismatch_with_query_metadata():
    retriever = InMemoryTopKRetriever()
    query = RetrievalQuery(
        query_text="test",
        query_vector=(1.0, 0.0, 0.0),  # size 3
        k=2,
        provider_id="fake_provider",
        model_id="fake_model",
        dimension=2  # metadata says 2
    )
    record = build_mock_record("table:test", (1.0, 0.0), dimension=2)
    with pytest.raises(EmbeddingNonRetryableError, match="Query vector size mismatch with query metadata"):
        retriever.retrieve(query, [record])

def test_retriever_rejects_record_provider_mismatch():
    retriever = InMemoryTopKRetriever()
    query = RetrievalQuery(
        query_text="test",
        query_vector=(1.0, 0.0),
    ...
    record = build_mock_record("table:test", (1.0, 0.0), dimension=2, provider_id="nvidia") # mismatch
    with pytest.raises(EmbeddingNonRetryableError, match="Embedding space provider mismatch"):
        retriever.retrieve(query, [record])

def test_retriever_rejects_record_model_mismatch():
    retriever = InMemoryTopKRetriever()
    query = RetrievalQuery(
        query_text="test",
        query_vector=(1.0, 0.0),
        k=2,
        provider_id="fake_provider",
        model_id="fake_model_v1",
        dimension=2
    )
    record = build_mock_record("table:test", (1.0, 0.0), dimension=2, model_id="fake_model_v2") # mismatch
    with pytest.raises(EmbeddingNonRetryableError, match="Embedding space model mismatch"):
        retriever.retrieve(query, [record])

def test_retriever_rejects_record_dimension_mismatch_with_query_metadata():
    retriever = InMemoryTopKRetriever()
    query = RetrievalQuery(
        query_text="test",
        query_vector=(1.0, 0.0),
    ...
    record = build_mock_record("table:test", (1.0, 0.0, 0.0), dimension=3) # mismatch
    with pytest.raises(EmbeddingNonRetryableError, match="Embedding space dimension mismatch"):
        retriever.retrieve(query, [record])
...
def test_retriever_rejects_unknown_record_id_prefix():
    retriever = InMemoryTopKRetriever()
    query = RetrievalQuery(
        query_text="test",
        query_vector=(1.0, 0.0),
    ...
    record = build_mock_record("database:test_db", (1.0, 0.0), dimension=2) # unknown prefix
    with pytest.raises(EmbeddingNonRetryableError, match="Unknown object type prefix in record ID"):
        retriever.retrieve(query, [record])

def test_retriever_rejects_malformed_record_id_without_object_id():
    retriever = InMemoryTopKRetriever()
    query = RetrievalQuery(
        query_text="test",
        query_vector=(1.0, 0.0),
    ...
    record1 = build_mock_record("table:", (1.0, 0.0), dimension=2) # empty object_id
    with pytest.raises(EmbeddingNonRetryableError, match="Malformed record ID"):
        retriever.retrieve(query, [record1])

    record2 = build_mock_record("table", (1.0, 0.0), dimension=2) # no colon
    with pytest.raises(EmbeddingNonRetryableError, match="Malformed record ID"):
        retriever.retrieve(query, [record2])
...
def test_retriever_maps_relationship_object_id_without_losing_type_confidence():
    """
    Verifies that the object_id of a relationship is correctly mapped
    to include type and confidence, exactly as split from the colon of the record ID.
    """
    retriever = InMemoryTopKRetriever()
    query = RetrievalQuery(
        query_text="test",
    ...
    rel_id = "relationship:orders.customer_id->customers.id|type=explicit|confidence=1.0"
    record = build_mock_record(rel_id, (1.0, 0.0), dimension=2)
    
    result = retriever.retrieve(query, [record])
    assert result.k_returned == 1
    assert result.candidates[0].object_id == "orders.customer_id->customers.id|type=explicit|confidence=1.0"
