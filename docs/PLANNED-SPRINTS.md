...
| 27.1 | Remaining Stage Span Builders · ▶️ *aktif — kalan 5 stage'in (INTENT/RETRIEVAL/PROMPT/GENERATION/VALIDATION) saf, duck-typed, secret-free span builder'ları + typed `*SpanDetail`'ler + testler. 27.0 çatısını tamamlar; driver-isolation korunur. Spec: `docs/superpowers/specs/2026-07-01-sprint-27.1-remaining-stage-span-builders-design.md`* |
| 27.1w | **Live Trace Wiring** · ⏳ *27.1'den bilinçli ERTELENDİ (sessiz düşürme yok): canlı pipeline telçekimi — `request_id` plumbing (`process_job_pipeline`→`run_pipeline`), per-stage `perf_counter`, `_capture_trace_on_exit`'te `EndToEndTrace` emission, `run_pipeline`'ın modüler stage'lere örülmesi. Kesin numara/slot implementasyon öncesi netleşecek; 27.2–27.10 kaymasın diye ayrı kalem.* |
