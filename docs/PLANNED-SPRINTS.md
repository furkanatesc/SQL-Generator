...
| 27.1w | **Live Trace Wiring** · ▶️ *sıradaki — 27.1'den bilinçli ERTELENDİ (sessiz düşürme yok): canlı pipeline telçekimi — `request_id` plumbing (`process_job_pipeline`→`run_pipeline`), per-stage `perf_counter`, `_capture_trace_on_exit`'te `EndToEndTrace` emission, `run_pipeline`'ın modüler stage'lere örülmesi. 27.1'den devreden 3 Minor da burada ele alınacak: `_GENERATION_ERROR_FINISH` boş-string over-eager ERROR; `RetrievalCandidateRef` score/rank Optional-vs-None; reason_code'un `_status_str`'i atlaması. Kesin numara/slot implementasyon öncesi netleşecek; 27.2–27.10 kaymasın diye ayrı kalem.* |
