...
- **Saf yaprak kuralı:** `backend/app/replay/` **yalnız stdlib** import eder. Yasak: `pydantic`, `fastapi`, `sqlglot`, `app.database`, **`app.trace.*` dahil her app modülü**, herhangi bir DB driver (`sqlite3` dahil). Gerekçe: `app/trace/__init__.py` tüm paketi eager import ettiği için `app.trace.end_to_end_trace`'i import etmek `sqlite3` + 19 app modülü çeker (2026-07-24'te ölçüldü). İhtiyaç duyulan stage/status literal'leri yerel sabit olarak tanımlanır.
