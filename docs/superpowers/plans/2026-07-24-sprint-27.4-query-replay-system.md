...
- **Saf yaprak kuralı:** `backend/app/replay/` yalnız **stdlib + `app.errors`** import eder. Yasak: `pydantic`, `fastapi`, `sqlglot`, `app.database`, **`app.trace.*`**, herhangi bir DB driver (`sqlite3` dahil). `app.trace` yasağının gerekçesi: `app/trace/__init__.py` tüm paketi eager import ettiği için `app.trace.end_to_end_trace`'i almak `sqlite3` + 19 app modülü çeker (2026-07-24'te ölçüldü); stage/status literal'leri bu yüzden yerel sabittir. `app.errors` ise ölçülerek izinli kılındı (yalnız kendi alt modüllerini çeker, sıfır driver) — güvenlik ekseninin dayandığı 27.2 registry kategorisi oradan okunur.

> **NOT (merge sonrası):** aşağıdaki Task 1/2 referans kod blokları, sprint sırasında review bulgularıyla değişen üç kararın ÖNCESİNİ gösterir (purity guard in-process + `_ALLOWED_APP_PREFIXES = ("app.replay",)`; güvenlik ekseni stage-tabanlı). Nihai tasarımın doğruluk kaynağı **spec**'tir (`docs/superpowers/specs/2026-07-24-sprint-27.4-query-replay-system-design.md`); shipped kod spec'i izler.
...
app.replay yaprak katmandir: YALNIZ stdlib import eder. app.trace.* bile yasaktir —
app/trace/__init__.py tum paketi eager import ettigi icin bir alt modulunu almak
sqlite3 + 19 app modulu cekiyor. Kardes guard'lar:
...
Stage/status literal'leri BILINCLI OLARAK yerel sabittir, app.trace'ten
import EDILMEZ: app/trace/__init__.py tum paketi eager import ettigi icin
app.trace.end_to_end_trace'i almak sqlite3 + 19 app modulu cekiyordu ve yaprak
saflik garantisini yok ediyordu (2026-07-24'te olculdu). Kaynak (tek dogruluk
noktasi): app/trace/end_to_end_trace.py TraceStageKind / TraceSpanStatus.
"""
...
Run: `cd backend && python -m pytest tests/replay -v`
Expected: PASS (15 test — Task 1'in 7'si + Task 2'nin 8'i). Purity guard da yeşil kalmalı — TEK BAŞINA da koştur: `python -m pytest tests/replay/test_replay_package_purity.py -q` (tam suite'te başka testler app.trace'i zaten yüklediği için sızıntı maskelenebilir).
...
def _input_available(job: Mapping[str, Any]) -> bool:
    ...
    if file_path:
        # Uretimde excel dali kosuldu; dosya yoksa o yol yeniden uretilemez
        # (natural_query dolu olsa bile).
...
def replay_job(job_id: str, *, pipeline, trace_store) -> ReplayResult:
    ...
    # URETIM GIBI: ikisi de gecilir, onceligi _stage_intent belirler
    # (excel_file_path varsa excel dali kosar). Bir job'in ikisine birden
    # sahip olmasi mumkun (/api/files/upload natural_query form alani).
    aqr, intent_error, _ = pipeline._stage_intent(
        excel_file_path=job.get("file_path"), natural_query=natural_query,
        log_callback=None)
    if intent_error:
        return compare_replay(
            job_id=job_id, baseline=baseline,
    ...
    # Baseline ile AYNI eksen tanimi (live_trace_assembly.py:152-160): yalniz
    # guvenlik nedeniyle reddedilen SQL, dogrulama ekseninde basarisiz SAYILMAZ.
...
- **`app.replay` purity guard'ının yapısal sınırı:** `sys.modules` cache'i üzerinden ölçüm yapar; sızdırılan modül zaten yüklüyse yakalanmaz — bu yüzden guard'ı tek başına da koşturmak gerekir (27.4'te tam bu maskeleme gerçek bir sızıntıyı gizledi). `errors`/`feedback` kardeşleriyle aynı bilinen sınır (27.3 M1).
