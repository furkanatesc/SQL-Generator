...
| (25.9) | **Oracle Adapter Contract STUB** — Oracle adapter'ın gerçek implementasyonu **yok**; sadece uyacağı sözleşme kilitlendi (`SQLOracleAdapterCapability` tüm execution flag'leri `False`, immutable contract, deterministic `NOT_IMPLEMENTED`/`REJECTED`, secret-safe result — sadece `sql_sha256`). cx_Oracle/oracledb/SQLAlchemy/JDBC/socket/DSN/TNS/wallet/env-secret **yasak** (testlerle kilitli). ⚠️ Gerçek Oracle execution Phase 10'da (29.3 adapter, 29.4 Docker/test harness). **Phase 6 (adapter stub'ları) burada kapanır.** *(PR pending)* |
