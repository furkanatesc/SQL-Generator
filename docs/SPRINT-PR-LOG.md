...
| PR | Açıklama |
|---|---|
| (26.0) | **SQL Permission Policy Contract** — backend'in ilk güvenlik/governance sözleşmesi. Bir SQL çalışmadan önce "izinli mi / deny mi / approval mı, neden?" sorusuna deterministik cevap veren contract. `SQLPermissionPolicyContract.evaluate()` → ALLOW/DENY/REQUIRES_APPROVAL + audit reason code (`default_deny`, `unknown_action`, `missing_subject`, …). **Fail-closed**: bilinmeyen action/resource, eksik subject/resource, konfigüre edilmemiş policy → hep DENY. Immutable request/result, secret-free result (context — connection string/password/token — sonuca sızmaz). Enforcement engine / tenant / RBAC / AuthN **yok** (sırasıyla 26.1–26.11). `backend/app/security/`. *(PR pending)* |
