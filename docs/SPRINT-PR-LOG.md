...
| (26.2) | **Read-Only Enforcement Hardening** — "bu SQL gerçekten read-only mi?" sorusunu dağınık string check'lerden çıkarıp tek, deterministik, contract-first enforcement katmanına bağlar (güvenlik üçgeninin SQL ayağı). `SQLReadOnlyEnforcementContract.enforce()` → ALLOW/DENY + audit reason code (`read_only_select`, `empty_sql`, `multi_statement`, `non_select_statement`, `forbidden_keyword`, `unsafe_procedure`, `unsafe_data_movement`, `invalid_sql_type`). **Fail-closed**: tek-statement SELECT (veya read-only `WITH … SELECT`) dışında her şey DENY; write/DDL/procedure/data-movement keyword'leri (string literal içinde bile) reddedilir; `SELECT … INTO` / `MERGE` / `COPY` → data-movement. Result ham SQL taşımaz — yalnızca `sql_sha256` + leading keyword (`normalized_prefix`). PostgreSQL adapter'ın `validate_read_only_select` gate'i artık bu ortak contract'a **delege** eder (forbidden keyword listesi tek yerde; mevcut rejection davranışı korunur, yalnızca sıkılaşır). Risk classifier / sensitive policy / PII / audit / approval, yeni adapter, gerçek execution, API/UI, tenant/RBAC/AuthN **yok**. `backend/app/security/sql_read_only_enforcement.py`. *(PR pending)* |
